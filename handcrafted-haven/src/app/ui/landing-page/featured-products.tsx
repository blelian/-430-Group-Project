...
export default async function FeaturedProducts() {
  // Get distinct non-null categories (normalize by trimming)
  ...
  // Build a string[] of available category names (filtered & trimmed)
  const availableCategories = productCategories
    .map((c) => c.category?.trim())
    .filter((c): c is string => typeof c === "string" && c.length > 0);

  if (availableCategories.length === 0) {
    // No populated categories => nothing to show
    return null;
  }
  ...
  const shuffledProducts = products.sort(() => Math.random() - 0.5).slice(0, 10); // show up to 10
